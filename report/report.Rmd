...
```{r, echo=FALSE}
arule <- icesControl(SSBdevs=sdevs$F, Fdevs=sdevs$SSB,
  Btrigger=42838, Blim=0, Ftarget=0.207, Fmin=0, recyrs=-2)
```
...
- A similar exercise was carried out with MPs in which the HCR also changed in the level at which fishing mortality will flatten as the stock declines, i.e. the minimum F is not 0.

```{r runs_minfs_hcrs, echo=FALSE}
include_graphics("runs_minfs_hcrs.png")
```

- The results are again compared with a zero fishing projection, but another one in which F is kept at some level, for example due to mixed fisheries catches, could be used instead. A simple call to 'fwd' with a 'fbar' target would provide that.

```{r runs_minfs, echo=FALSE}
include_graphics("runs_minfs.png")
```
...
- A number of functions are currently defined in the `utilities.R` file. any function of general application will be incorporated into any of the FLKR packages being used. ICES-specific functions should be integrated into an MSE-related package, or on any of the existing ICEs R package, if suitable.
