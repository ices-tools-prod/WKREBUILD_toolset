...
- INSTALL from r-universe and CRAN

# NOTE: to remove last year use `window(run, end=-1)`

# [`data.R`](https://github.com/iagomosqueira/WKREBUILD_toolset/blob/main/data.R) - Building an Operating Model
...
stochasticity, process and estimation uncertainty, and correlated
errors (Kell et al., 2005; ICES, 2013; Punt et al., 2016)
...
Uncertainty in past dynamics and initial conditions might have been quantified, for example by employing the McMC sampling procedure available in the stock assessment model, by bootstrap of model inputs, or through a ensemble of model fits. If that is the case, the conditioning steps in `data.R` should be adapted accordingly. 

- `runmc`: an example stock assessment run where parameter uncertainty is already included. This was obtained by employing the `mcmc` option of ADMB.

- `tac`: The total catch to be applied to the stock in the intermediate year. In this case this reflects an expected level of uptake of the 2023 TAC.

## data.R

This file contains the Operating Model conditioning, limited here to a minimum level of consideration of uncertainty.
...
run <- fwd(run, sr=rec(run)[, ac(2013:2022)],
  fbar=fbar(run)[, ac(2013:2022)] * 1.6)

plot(srdevs) +
  geom_line(aes(y=mean(data)), linetype=2)

## Stock-recruit relationships

aic_srpars <- bootstrapSR(window(run, end=iy - 1), iters=it,
  models=c("bevholt", "segreg"), method="aic")

loglik_srpars <- bootstrapSR(window(run, end=iy - 1), iters=it,
  models=c("bevholt", "segreg"), method="loglik")

(plot_bootstrapSR(fits, srpars) + ggtitle("Highest lkhd")) +
(plot_bootstrapSR(fits, aic_srpars) + ggtitle("Relative lkhd")) +
(plot_bootstrapSR(fits, loglik_srpars) + ggtitle("logLik probability"))

dat <- rbindlist(list(lkhd=model.frame(srpars), relkhd=model.frame(aic_srpars), prob=model.frame(loglik_srpars)), idcol="method")

dcast(dat[, .(a,b,spr0,logLik,sigmaR,R0,rho,B0,m, method)], method + m ~ ., fun=mean, na.rm=TRUE)

dat <- rbindlist(list(lkhd=as.data.frame(srpars), relkhd=as.data.frame(aic_srpars), prob=as.data.frame(loglik_srpars)), idcol="method")

ggplot(dat[params %in% c('a','b','R0','B0', 'm')], aes(x=data)) +     
  geom_histogram() +
  facet_grid(method ~ params, scales='free')

# SELECT by U(0,1) ~ P(model)
srpars <- bootstrapSR(window(run, end=iy - 1), iters=it,
  models=c("bevholt", "ricker", "segreg"), method="probability")

# OM from McMC run
omc <- fwdWindow(FLom(stock=runmc, refpts=refpts, model='mixedsrr',
  params=srpars, deviances=srdevs), end=fy)
